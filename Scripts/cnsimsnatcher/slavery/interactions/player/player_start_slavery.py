...
log = CommonLogRegistry.get().register_log(ModInfo.get_identity(), 'ss_start_enslave')
...
class SSSlaveryStartSlaveryInteraction(CommonImmediateSuperInteraction):
    ...
    def on_test(cls, interaction_sim: Sim, interaction_target: Any, interaction_context: InteractionContext, **kwargs) -> TestResult:
        ...
        if not SSSlaveryUtils().can_engage_in_slavery(sim_info) or not SSSlaveryUtils().can_engage_in_slavery(target_sim_info):
            log.debug('Failed, Active Sim or Target Sim cannot engage in slavery.')
            return TestResult.NONE
        if sim_info is target_sim_info:
            log.debug('Failed, Cannot enslave self')
            return TestResult.NONE
        if not SSSlaveryUtils().is_allowed_to_enslave_others(sim_info):
            log.debug('Failed, Active Sim not allowed to enslave other sims.')
            return TestResult.NONE
        if not SSSlaveryUtils().is_allowed_to_be_enslaved(target_sim_info):
            log.debug('Failed, Target Sim is not allowed to be enslaved.')
            return TestResult.NONE
        ...
        if SSSlaveryStateUtils().has_masters(target_sim_info):
            log.debug('Failed, Target Sim already has masters.')
            return TestResult.NONE
        if not SSAbductionStateUtils.has_been_abducted(target_sim_info):
            log.debug('Failed, Target Sim must already have been abducted to be enslaved.')
            return TestResult.NONE
        log.debug('Success! The Sim can enslave the Target Sim.')
    ...
    def on_started(self, interaction_sim: Sim, interaction_target: Any) -> bool:
        ...
        log.debug('Queuing "Attempt To Enslave" interaction.')
