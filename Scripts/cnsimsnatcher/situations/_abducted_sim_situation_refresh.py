...
class _SSAbductionRefreshSituation(CommonService, HasLog):
    ...
    def refresh_status_of_sim(self, captive_sim_info: SimInfo) -> bool:
        ...
        try:
            self.log.format_with_message('Attempting to refresh enslaved status of \'{}\'.'.format(captive_sim_name))
            ...
            if sim_instance is None:
                ...
                return False
            captor_sim_info_list = self._state_utils.get_captors(captive_sim_info, instanced_only=True)
            if not captor_sim_info_list:
                self.log.debug('Failed, no captors found.')
                return False
            for captor_sim_info in captor_sim_info_list:
                if not CommonHouseholdUtils.is_part_of_active_household(captor_sim_info):
                    self.log.debug('Failed, the captor sim is not a part of the active household.')
                    continue
