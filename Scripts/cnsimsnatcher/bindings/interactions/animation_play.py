...
class SSAnimationPlayInteraction(CommonBaseSuperInteraction, CommonConstrainedSuperInteraction):
    ...
    def _setup_asm_default(self, interaction_sim: Sim, interaction_target: Any, interaction_asm: NativeAsm, *args, **kwargs) -> Union[bool, None]:
        ...
        interaction_asm.set_actor(actor_param, interaction_sim)

        game_object = CommonObjectUtils.get_game_object(interaction_target)
        if game_object is not None:
            interaction_asm.set_parameter('object_animation_name', '')
            interaction_asm.set_actor('o', game_object)
